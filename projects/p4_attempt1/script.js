...
var CardGame = function(targetId)
{
  ...
  // {
    // cards[id].firstChild.src = "xx.jpg";
  //   with(cards[id].style) {
  ...
  {
    hideCard(id);
    cards[id].firstChild.src = "xx.jpg"; //matched = true;
    with(cards[id].style) {
      zIndex = "1000";
      top = "100px";
      left = "-140px";
  ...
  var showCard = function(id) // turn card face up, check for match
  {
    ...
    cards[id].firstChild.src = "xx.jpg"; //+ card_value[id]; // + "xx.jpg";
    with(cards[id].style) {
      WebkitTransform = MozTransform = OTransform = msTransform = "scale(1.0) rotate()";
    }
  ...
  card.innerHTML = "<img src=\"xxx.jpg\">";

  for(var i=0; i < 16; i++) {
    var newCard = card.cloneNode(true);

    newCard.fromtop = 15 + 150 * Math.floor(i/4);
    newCard.fromleft = 70 + 200 * (i%4);
    (function(idx) {
